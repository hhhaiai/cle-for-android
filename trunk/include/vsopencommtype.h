...
//=========define os type, total kinds is 16
typedef struct{
    VS_UINT16 OsType;
...
#ifdef SRPCPLUSPLUS
      StructOfLocalVSFont(VS_INT32 In_Height,VS_INT32 In_Size,VS_UINT8 In_CharSet,VS_UINT8 In_Style,VS_CHAR *In_Name)
      {
          Height = In_Height;Size = In_Size;CharSet = In_CharSet;Style=In_Style;
          strncpy(Name,In_Name,VSFONT_NAMELENGTH);
          Name[VSFONT_NAMELENGTH-1] = 0;
      };
      StructOfLocalVSFont(){};
#endif
}VS_FONT;
...
#ifdef SRPCPLUSPLUS
    StructOfLocalVString(VS_CHAR *InBuf){Buf=InBuf;};
    StructOfLocalVString(){};
#endif
}VS_VSTRING;
...
#define VSTYPE_IGNORE           255   //---Æ½does not exist within the Platform
